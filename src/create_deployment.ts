...
const rawSDL = fs.readFileSync(path.resolve(__dirname, "./required-files/example.sdl.yml"), "utf8");
...
async function walletFromMnemonic(mnemonic: string) {
  try {
    ...
    console.error('Could not create wallet from mnemonic, have you updated ".env" file?');
    throw error;
  }
}
